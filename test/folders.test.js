...
describe('Noteful API - Folders', function () {
  ...
  describe('GET /api/folders', function () {
    ...
    it('should catch errors and respond properly', function () {
      const dbPromise = sandbox.stub(Folder.schema.options.toJSON, 'transform').throws('FakeError');
      const apiPromise = chai.request(app)
        .get('/api/folders')
        .set('Authorization', `Bearer ${token}`);
      ...
      return Promise.all([dbPromise, apiPromise])
        .then(res => {
          // console.log(res);
          // console.log(res[1]);
          expect(res[1]).to.have.status(500);
          expect(res[1]).to.be.json;
          // console.log(res.body);
          expect(res[1].body).to.be.a('object');
          expect(res[1].body.message).to.equal('Internal Server Error');
        });
    });
